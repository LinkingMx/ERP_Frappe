(() => {
  ...
  $.extend(erpnext, {
    ...
    toggle_serial_batch_fields(frm) {
      var _a;
      let hide_fields = cint((_a = frappe.user_defaults) == null ? void 0 : _a.enable_serial_and_batch_no_for_item) === 0 ? 1 : 0;
      if (!hide_fields) {
        return;
      }
      ...
      if ((sn_field == null ? void 0 : sn_field.length) && sn_field[0].hidden === 1) {
        return;
      }
  ...
  erpnext.utils.update_child_items = function(opts) {
    ...
    if (["Purchase Order", "Sales Order"].includes(frm.doc.doctype) && frm.doc.is_subcontracted && !frm.doc.is_old_subcontracting_flow) {
      fields.push(
        {
          fieldtype: "Link",
          fieldname: "fg_item",
  ...
  // frappe-html:/workspace/apps/erpnext/erpnext/public/js/templates/visual_plant_floor_template.html
...
{% }); %}`;
  ...
  erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
    ...
    calculate_totals() {
      ...
      if (["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "POS Invoice"].includes(
        ...
        this.frm.doc.base_grand_total = flt(
          this.frm.doc.taxes_and_charges_added || this.frm.doc.taxes_and_charges_deducted ? flt(this.frm.doc.grand_total * this.frm.doc.conversion_rate) : this.frm.doc.base_net_total
        );
    ...
    set_discount_amount() {
      if (this.frm.doc.additional_discount_percentage) {
        this.frm.doc.discount_amount = flt(
          flt(this.frm.doc[frappe.scrub(this.frm.doc.apply_discount_on)]) * this.frm.doc.additional_discount_percentage / 100,
    ...
    calculate_outstanding_amount(update_paid_amount) {
      ...
      if (["Sales Invoice", "POS Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
        ...
        if (["Sales Invoice", "POS Invoice"].includes(this.frm.doc.doctype)) {
          let total_amount_for_payment = this.frm.doc.redeem_loyalty_points && this.frm.doc.loyalty_amount ? flt(
    ...
    set_default_payment(total_amount_to_pay, update_paid_amount) {
      ...
      if (this.frm.doc.is_pos && cint(this.frm.set_default_payment) && (update_paid_amount === void 0 || update_paid_amount)) {
        $.each(this.frm.doc["payments"] || [], function(index, data) {
  ...
  erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
    constructor() {
      super(...arguments);
      __publicField(this, "make_payment_request_with_schedule", async function() {
        let frm = this.frm;
      ...
      frappe.run_serially([
        () => {
          if (show_batch_dialog && !frappe.flags.trigger_from_barcode_scanner)
            return frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
    ...
    set_dynamic_labels() {
      ...
      var company_currency = this.get_company_currency();
      if (this._last_currency === this.frm.doc.currency && this._last_price_list_currency === this.frm.doc.price_list_currency) {
        return;
      }
        ...
        if (data && data.apply_rule_on_other_items && JSON.parse(data.apply_rule_on_other_items)) {
          ...
          me.frm.doc.items.forEach((d) => {
            if (JSON.parse(data.apply_rule_on_other_items).includes(d[data.apply_rule_on])) {
              for (var k2 in data) {
                if (fields.includes(k2) && data[k2] && (data.price_or_product_discount === "Price" || k2 === "pricing_rules")) {
      ...
      $.each(["company", "customer"], function(i, fieldname) {
        if (frappe.meta.has_field(me.frm.doc.doctype, fieldname) && !["Purchase Order", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
          if (!me.frm.doc[fieldname]) {
            frappe.msgprint(
              __("Please specify") + ": " + __(frappe.meta.get_label(me.frm.doc.doctype, fieldname, me.frm.doc.name)) + ". " + __("It is needed to fetch Item Details.")
            );
            valid = false;
          }
        }
  ...
  // frappe-html:/workspace/apps/erpnext/erpnext/public/js/templates/item_selector.html
...
`;
  ...
  // frappe-html:/workspace/apps/erpnext/erpnext/public/js/templates/item_quick_entry.html
...
`;
  ...
  // frappe-html:/workspace/apps/erpnext/erpnext/public/js/templates/call_link.html
...
`;
  ...
  // frappe-html:/workspace/apps/erpnext/erpnext/public/js/templates/crm_activities.html
...
.form-footer {
  ...
  // frappe-html:/workspace/apps/erpnext/erpnext/public/js/templates/crm_notes.html
      ...
      frappe.ui.form.on(doctype, {
        setup: function(frm) {
          $(frm.wrapper).on("grid-row-render", function(e, grid_row) {
            if (["Sales Taxes and Charges", "Purchase Taxes and Charges"].includes(
  ...
  erpnext.financial_statements = {
    ...
    formatter: function(value, row, column, data, default_formatter, filter) {
      ...
      if (get_filter_value("report_template"))
        ...
        return erpnext.financial_statements._format_standard_report(...report_params);
    },
    ...
    _parse_column_info: function(fieldname, data) {
      ...
      const periodKeys = ((_a = data._segment_info) == null ? void 0 : _a.period_keys) || [];
    ...
    _format_custom_account_column: function(value, data, formatting, column, default_formatter, row) {
      ...
      if (!value)
        return "";
      const should_link_to_ledger = formatting.is_detail || formatting.account_filters && formatting.child_accounts && formatting.child_accounts.length;
      if (should_link_to_ledger) {
        const glData = {
          account: Array.isArray(formatting.child_accounts) && formatting.child_accounts.length ? formatting.child_accounts : (_a = formatting.account) != null ? _a : value,
        ...
        column.link_onclick = "erpnext.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
    ...
    _format_custom_value_column: function(value, data, formatting, column, default_formatter, row) {
      if (formatting.is_blank_line)
        return "";
      const col = __spreadValues({}, column);
    ...
    _format_standard_report: function(value, row, column, data, default_formatter, filter) {
      ...
      value = default_formatter(value, row, column, data);
      if (data && (!data.parent_account && !data.parent_section || data.is_group_account || data.is_group)) {
      ...
      if (report.page) {
        ...
        report.page.add_custom_menu_item(views_menu, __("Balance Sheet"), function() {
          ...
          frappe.set_route("query-report", "Balance Sheet", {
            company: filters2.company,
            filter_based_on: filters2.filter_based_on,
            period_start_date: filters2.period_start_date,
            period_end_date: filters2.period_end_date,
            from_fiscal_year: filters2.from_fiscal_year,
            to_fiscal_year: filters2.to_fiscal_year,
            periodicity: filters2.periodicity,
            presentation_currency: filters2.presentation_currency,
            cost_center: filters2.cost_center,
            project: filters2.project
          });
        });
...
//# sourceMappingURL=erpnext.bundle.SSAWX4UJ.js.map
